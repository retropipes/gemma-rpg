...
public class BattleLogic {
	...
	// Generate Friends
	BattleCharacter[] friends = PartyManager.getParty().getBattleCharacters();
	// Generate Enemies
	...
	if (!this.resultDoneAlready) {
		...
		if (this.bd.findBattler("The Final Boss") != -1) {
		    // Boss battle
		    CommonDialogs.showTitledDialog("The Final Boss has fallen!", "Final Victory!");
		    ...
		    Creature enemy = this.bd.getBattlers()[this.bd.findBattler(this.bd.getNameOfPartyEnemy())]
			    .getTemplate();
		    PartyManager.getParty().distributeVictorySpoils(this.vsd, enemy.getLevel());
		    Gemma.getApplication().getGameManager().addToScore(Math.max(1,
	    ...
	    // Level Up Check
	    ArrayList<InternalScript> levelUpScripts = PartyManager.getParty().checkPartyLevelUp();
	...
	if (active != null && aicontext != null && ai != null) {
		...
		int x = this.bd.getActiveCharacter().getTemplate().getAI().getMoveX();
		int y = this.bd.getActiveCharacter().getTemplate().getAI().getMoveY();
	...
	int action = this.auto.getNextAction(this.bd.getBattlerAIContexts()[index]);
	switch (action) {
	case AIRoutine.ACTION_MOVE:
	...
	RandomRange randX = new RandomRange(0, this.bd.getBattleMap().getRows() - 1);
	RandomRange randY = new RandomRange(0, this.bd.getBattleMap().getColumns() - 1);
	int rx, ry;
	// Set Character Locations
	...
	if (res != -1) {
	    ...
	    // AI Check
	    if (this.bd.getActiveCharacter().getTemplate().hasAI() && !Support.inDebugMode()) {
		// Run AI
		this.waitForAI();
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlers()[x].getTeamID() == teamID) {
		    boolean res = this.bd.getBattlers()[x].getTemplate().isAlive();
		    if (res) {
			return true;
		    }
		}
	...
	if (this.bd.getActiveCharacter().getCurrentAP() > 0) {
	    ...
	    ArrowTask at = new ArrowTask(x, y, this.bd.getActiveCharacter().getTemplate().getFaith().getFaithID(),
		    this.bd);
	    at.start();
	} else {
	...
	if (hit != null && !hit.getTemplate().isAlive()) {
		...
		this.vsd.setExpPerMonster(this.bd.findBattler(hit.getName()) - partySize,
			hit.getTemplate().getExperience());
	    }
	    // Update Prestige
	    BattleDefinitions.killedEnemy(this.bd.getActiveCharacter().getTemplate());
	    ...
	    this.bd.getBattleMap().setBattleCell(new Empty(), hit.getX(), hit.getY());
	}
	// Check result
	int currResult = this.getResult();
	if (currResult != BattleResults.IN_PROGRESS) {
    ...
    private boolean updatePositionInternal(int x, int y, boolean useAP, BattleCharacter active) {
	...
	if (next != null && nextGround != null && currGround != null) {
		...
		if ((useAP && this.getActiveActionCounter() >= MapObject.getBattleAPCost()) || !useAP) {
		    // Move
		    MapObject obj1 = null;
		    MapObject obj2 = null;
		    MapObject obj3 = null;
		    ...
		    this.decrementActiveActionCounterBy(MapObject.getBattleAPCost());
			...
			InternalScriptRunner.runScript(nextGround.getBattlePostMoveScript(active));
		    }
		} else {
		...
		if (next.isOfType(TypeConstants.TYPE_BATTLE_CHARACTER)) {
			...
			if (bc.getTeamID() == active.getTeamID()) {
				...
				int confirm = CommonDialogs.showConfirmDialog("Attack Friend?", "Battle");
			...
			if (LocalPreferencesManager.getRandomBattleEnvironment()) {
			    // Run any script attached to ground
			    InternalScriptRunner.runScript(currGround.getBattlePostMoveScript(active));
			}
			// Do damage
			this.computeDamage(enemy, active.getTemplate());
			// Handle low health for party members
			if (enemy.isAlive() && enemy.getTeamID() == Creature.TEAM_PARTY
				&& enemy.getCurrentHP() <= enemy.getMaximumHP() * 3 / 10) {
			    SoundManager.playSound(GameSoundConstants.SOUND_LOW_HEALTH);
			}
			// Handle enemy death
			if (!enemy.isAlive()) {
				...
				// Update victory spoils
				int partySize = PartyManager.getParty().getActivePCCount();
				this.vsd.setExpPerMonster(this.bd.findBattler(enemy.getName()) - partySize,
					enemy.getExperience());
			    }
			    // Update Prestige
			...
			if (!active.getTemplate().isAlive()) {
			    // Update Prestige
			    BattleDefinitions.killedInBattle(active.getTemplate());
			    // Play death sound
			    SoundManager.playSound(GameSoundConstants.SOUND_DEATH);
			    // Remove effects from dead character
		...
		int confirm = CommonDialogs.showConfirmDialog("Embrace Cowardice?", "Battle");
		if (confirm != JOptionPane.YES_OPTION) {
	...
	if (this.getActiveSpellCounter() > 0) {
		...
		boolean success = SpellCaster.selectAndCastSpell(this.bd.getActiveCharacter().getTemplate(),
		...
		Spell sp = this.bd.getActiveCharacter().getTemplate().getAI().getSpellToCast();
		boolean success = SpellCaster.castSpell(sp, this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.decrementActiveSpellCounter();
		    // Update Prestige
	...
	if (this.getActiveItemCounter() > 0) {
	    if (!this.bd.getActiveCharacter().getTemplate().hasAI() || Support.inDebugMode()) {
		// Active character has no AI, or AI is turned off
		boolean success = CombatItemChucker.selectAndUseItem(this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		...
		// Active character has AI, and AI is turned on
		CombatItem cui = this.bd.getActiveCharacter().getTemplate().getAI().getItemToUse();
		boolean success = CombatItemChucker.useItem(cui, this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.bd.getActiveCharacter().modifyItems(1);
		}
	...
	if (this.getActiveActionCounter() > 0) {
	    ...
	    int drainChance;
	    int drainAmount;
	    this.bd.getActiveCharacter().modifyAP(BattleLogic.DRAIN_ACTION_POINTS);
		...
		RandomRange drained = new RandomRange(0, activeEnemy.getCurrentMP());
	...
	this.updateStatsAndEffects();
	if (this.battleGUI.getOutputFrame().isVisible() && this.bd.getActiveCharacter() != null) {
	    this.battleGUI.getViewManager().setViewingWindowCenterX(this.bd.getActiveCharacter().getY());
	    this.battleGUI.getViewManager().setViewingWindowCenterY(this.bd.getActiveCharacter().getX());
	    this.redrawBattle();
	}
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		String effectMessages = this.bd.getBattlers()[x].getTemplate().getAllCurrentEffectMessages();
		...
		if (!this.bd.getBattlers()[x].getTemplate().isAlive()) {
			...
			this.vsd.setExpPerMonster(x - partySize,
				this.bd.getBattlers()[x].getTemplate().getExperience());
		    }
		    // Play death sound
		    SoundManager.playSound(GameSoundConstants.SOUND_DEATH);
		    // Set dead character to inactive
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlerAIContexts()[x] != null
			...
			&& !Support.inDebugMode() && this.bd.getBattlers()[x].isActive()
