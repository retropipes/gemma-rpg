...
class BattleGUI {
	...
	this.battleFrame.setJMenuBar(Gemma.getApplication().getMenuManager().getMainMenuBar());
    }
	...
	if (this.battleFrame.isVisible()) {
		...
		for (y = yView; y <= ylView; y++) {
		    ...
		    yFix = y - yView;
		    try {
			BufferedImageIcon icon1 = bd.getBattleMap().getBattleGround(y, x).battleRenderHook();
			BufferedImageIcon icon2 = bd.getBattleMap().getBattleCell(y, x).battleRenderHook();
			this.drawGrid.setImageCell(ImageTransformer.getCompositeImage(icon1, icon2), xFix, yFix);
		    } catch (final ArrayIndexOutOfBoundsException | NullPointerException ae) {
			this.drawGrid.setImageCell(ImageManager.getMapImage("Void", "Void", null), xFix, yFix);
		    }
		}
	    }
    ...
    void redrawOneBattleSquare(BattleDefinitions bd, int x, int y, MapObject obj3) {
	// Draw the battle, if it is visible
	if (this.battleFrame.isVisible()) {
	    try {
		int xFix, yFix;
		...
		xFix = y - xView;
		yFix = x - yView;
		BufferedImageIcon icon1 = bd.getBattleMap().getBattleGround(x, y).battleRenderHook();
		BufferedImageIcon icon2 = bd.getBattleMap().getBattleCell(x, y).battleRenderHook();
		BufferedImageIcon icon3 = obj3.battleRenderHook();
		this.drawGrid.setImageCell(ImageTransformer.getCompositeImage(icon1, icon2, icon3), xFix, yFix);
		this.battlePane.repaint();
	    } catch (final ArrayIndexOutOfBoundsException | NullPointerException ae) {
		// Do nothing
	    }
	...
	this.battlePane.setLayout(new GridLayout(BattleViewingWindowManager.getViewingWindowSize(),
		BattleViewingWindowManager.getViewingWindowSize()));
	this.battleFrame.setResizable(false);
	this.drawGrid = new DrawGrid(BattleViewingWindowManager.getViewingWindowSize());
	for (int x = 0; x < BattleViewingWindowManager.getViewingWindowSize(); x++) {
	    for (int y = 0; y < BattleViewingWindowManager.getViewingWindowSize(); y++) {
	...
	private void handleMovement(final KeyEvent e) {
		...
		if (bg.eventHandlersOn) {
			...
			int res = CommonDialogs.showConfirmDialog("Are you sure you want to attack yourself?",
				"Battle");
