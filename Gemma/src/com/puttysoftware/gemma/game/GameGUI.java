...
class GameGUI {
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
		    ...
		    visible = app.getScenarioManager().getMap().isSquareVisible(u, v, y, x);
		    try {
			if (visible) {
			    MapObject obj1, obj2;
			    obj1 = m.getCell(y, x, m.getPlayerLocationZ(), MapConstants.LAYER_GROUND);
			    obj2 = m.getCell(y, x, m.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
			    String name1, name2;
			    name1 = obj1.gameRenderHook(y, x, m.getPlayerLocationZ(),
				    app.getScenarioManager().getMap());
			    name2 = obj2.gameRenderHook(y, x, m.getPlayerLocationZ(),
				    app.getScenarioManager().getMap());
			    boolean hasNote = m.hasNote(y, x, m.getPlayerLocationZ());
			    if (hasNote) {
				this.drawGrid
					.setImageCell(
						ImageTransformer.getCompositeImage(
							ImageManager.getMapImage(name1, obj1.getGameName(),
								obj1.getGameTemplateTransform()),
							ImageManager.getMapImage(name2, obj2.getGameName(),
								obj2.getGameTemplateTransform()),
							ImageManager.getMapImage("Map Note", "Map Note", null)),
						xFix, yFix);
			    } else {
				this.drawGrid.setImageCell(ImageTransformer.getCompositeImage(
					ImageManager.getMapImage(name1, obj1.getGameName(),
						obj1.getGameTemplateTransform()),
					ImageManager.getMapImage(name2, obj2.getGameName(),
						obj2.getGameTemplateTransform())),
			    ...
			    this.drawGrid.setImageCell(ImageManager.getMapImage("Darkness", "Darkness", null), xFix,
				    yFix);
			}
		    } catch (final ArrayIndexOutOfBoundsException | NullPointerException ae) {
			this.drawGrid.setImageCell(ImageManager.getMapImage(new EmptyVoid().gameRenderHook(y, x,
				m.getPlayerLocationZ(), app.getScenarioManager().getMap()), "Void", null), xFix, yFix);
		    }
		}
	    }
	...
	Map m = Gemma.getApplication().getScenarioManager().getMap();
	this.vwMgr.setViewingWindowLocationX(m.getPlayerLocationY() - GameViewingWindowManager.getOffsetFactor());
	this.vwMgr.setViewingWindowLocationY(m.getPlayerLocationX() - GameViewingWindowManager.getOffsetFactor());
    }
	...
	this.outputPane.setLayout(new GridLayout(GameViewingWindowManager.getViewingWindowSize(),
	...
	this.drawGrid = new DrawGrid(GameViewingWindowManager.getViewingWindowSize());
	for (int x = 0; x < GameViewingWindowManager.getViewingWindowSize(); x++) {
		...
		this.drawGrid.setImageCell(ImageManager.getMapImage("Darkness", "Darkness", null), x, y);
	    }
	}
	this.outputPane.updateGrid(this.drawGrid);
	this.borderPane.add(this.outputPane, BorderLayout.CENTER);
