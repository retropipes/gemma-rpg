...
public final class InternalScriptRunner {
	...
	try {
		...
		for (int x = 0; x < totAct; x++) {
			...
			Shop shop = Gemma.getApplication().getGenericShop(shopType);
			if (shop != null) {
			    shop.showShop();
			} else {
			...
			if (moveType == InternalScriptConstants.MOVE_RELATIVE) {
				...
				gm.updatePositionRelativeNoEvents(moveX, moveY, moveZ);
			    }
			} else {
			    if (eventFlag) {
			...
			MapObject swapObj1 = Gemma.getApplication().getObjects().getNewInstanceByName(swap1);
			MapObject swapObj2 = Gemma.getApplication().getObjects().getNewInstanceByName(swap2);
			Gemma.getApplication().getScenarioManager().getMap().findAllObjectPairsAndSwap(swapObj1,
				swapObj2);
			break;
		    case REDRAW:
			...
			Gemma.getApplication().getGameManager().addToScore(points);
			break;
		    case RANDOM_CHANCE:
			// Random Chance
			...
			new Thread("Battle") {
				...
				try {
				    ...
				    Gemma.getApplication().getBattle().doFixedBattle(Map.getTemporaryBattleCopy(),
					    battle);
				} catch (Exception e) {
			...
			Gemma.getApplication().getGameManager().goToLevelRelative(rDestLevel);
			break;
		    case UPDATE_GSA:
			int gsaMod = se.getActionArg(0).getInteger();
			Gemma.getApplication().getScenarioManager().getMap().rebuildGSA(gsaMod);
			break;
		    default:
	...
	InternalScriptActionCode code = se.getActionCode();
	int rargc = InternalScriptConstants.ARGUMENT_COUNT_VALIDATION[code.ordinal()];
	int aargc;
	try {
	...
	Class<?>[] rargt = InternalScriptConstants.ARGUMENT_TYPE_VALIDATION[code.ordinal()];
	if (rargt != null) {
