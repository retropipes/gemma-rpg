...
public class GameLogic {
	...
	Gemma.getApplication().getScenarioManager().getMap().updateVisibleSquares(x, y, z);
    }
	...
	try {
	    ...
	    try {
		nextBelow = m.getCell(px + x, py + y, pz + z, MapConstants.LAYER_GROUND);
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		nextBelow = new Empty();
	    }
	    try {
		nextAbove = m.getCell(px + x, py + y, pz + z, MapConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		nextAbove = new Wall();
	    }
	    try {
	...
	if (proceed) {
	    ...
	    try {
		if (GameLogic.checkSolid(pz + z, this.savedMapObject, below, nextBelow, nextAbove)) {
			...
			InternalScriptRunner.runScript(groundInto.getPostMoveScript(false, px, py, pz));
			if (!this.savedMapObject.isOfType(TypeConstants.TYPE_PASS_THROUGH)) {
			    InternalScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, px, py, pz));
			}
		    } else {
			InternalScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, px, py, pz));
		    }
		    ...
		    GameLogic.fireMoveFailedActions(px + x, py + y, pz + z, this.savedMapObject, below, nextBelow,
			    nextAbove);
		    proceed = false;
		}
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		...
		m.setCell(new Player(), m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
	    ...
	    InternalScriptRunner.runScript(MapObject.getMoveFailedScript(false, px + x, py + y, pz + z));
	    proceed = false;
	}
	if (redrawsSuspended) {
	    // Redraw post-suspend
	...
	// Post-move actions
	ArrayList<InternalScriptArea> areaScripts = app.getScenarioManager().getMap()
	...
	if (proceed) {
	    ...
	    this.getViewManager().saveViewingWindow();
	    try {
		if (GameLogic.checkSolid(z + pz, this.savedMapObject, below, nextBelow, nextAbove)) {
		    m.setCell(this.savedMapObject, px, py, pz, MapConstants.LAYER_OBJECT);
		    m.offsetPlayerLocationX(x);
		    m.offsetPlayerLocationY(y);
		    m.offsetPlayerLocationZ(z);
	...
	int py = m.getPlayerLocationY();
	int pz = m.getPlayerLocationZ();
	GenericCharacter player = (GenericCharacter) m.getCell(px, py, pz, MapConstants.LAYER_OBJECT);
	...
	Map m = Gemma.getApplication().getScenarioManager().getMap();
	m.findStart();
	m.setPlayerLocation(m.getStartColumn(), m.getStartRow(), m.getStartFloor(), m.getActiveLevelNumber());
	this.resetViewingWindow();
	this.redrawMap();
    }

    private static boolean checkSolid(final int z, final MapObject inside, final MapObject below,
    ...
    private static void fireMoveFailedActions(final int x, final int y, final int z, final MapObject inside,
	    final MapObject below, final MapObject nextBelow, final MapObject nextAbove) {
	Map m = Gemma.getApplication().getScenarioManager().getMap();
	boolean insideSolid = inside.isConditionallySolid(m, z);
	boolean belowSolid = below.isConditionallySolid(m, z);
	boolean nextBelowSolid = nextBelow.isConditionallySolid(m, z);
	boolean nextAboveSolid = nextAbove.isConditionallySolid(m, z);
	if (insideSolid) {
	    InternalScriptRunner.runScript(MapObject.getMoveFailedScript(false, x, y, z));
	}
	if (belowSolid) {
	    InternalScriptRunner.runScript(MapObject.getMoveFailedScript(false, x, y, z));
	}
	...
	try {
		...
		m.setCell(this.savedMapObject, m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MapConstants.LAYER_OBJECT);
		m.setPlayerLocation(x, y, z, 0);
		this.getViewManager()
			.setViewingWindowLocationX(m.getPlayerLocationY() - GameViewingWindowManager.getOffsetFactor());
		this.getViewManager()
			.setViewingWindowLocationY(m.getPlayerLocationX() - GameViewingWindowManager.getOffsetFactor());
		this.savedMapObject = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MapConstants.LAYER_OBJECT);
		m.setCell(new Player(), m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MapConstants.LAYER_OBJECT);
		app.getScenarioManager().setDirty(true);
		InternalScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, x, y, z));
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    m.restorePlayerLocation();
	    this.getViewManager().restoreViewingWindow();
	...
	GameLogic.fireStepActions(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ());
	this.redrawMap();
    }
	...
	if (!levelExists && m.isLevelOffsetValid(level)) {
	    ...
	    m.addLevel(Support.getGameMapSize(), Support.getGameMapSize(), Support.getGameMapFloorSize());
	...
	if (PreferencesManager.getMusicEnabled(PreferencesManager.MUSIC_EXPLORING)) {
	    ...
	    MusicManager.playMusic(MusicConstants.MUSIC_EXPLORING, m.getPlayerLocationZ() / 4);
	}
    }
	...
	Map m = app.getScenarioManager().getMap();
	m.switchLevel(level);
	m.setPlayerLocation(m.getStartColumn(), m.getStartRow(), m.getStartFloor(), level);
    }
	...
	final int yOffset = this.getViewManager().getViewingWindowLocationY()
		- GameViewingWindowManager.getOffsetFactor();
	final int destX = x / ImageManager.getGraphicSize() + this.getViewManager().getViewingWindowLocationX()
		- xOffset + yOffset;
	final int destY = y / ImageManager.getGraphicSize() + this.getViewManager().getViewingWindowLocationY()
		+ xOffset - yOffset;
	final int destZ = m.getPlayerLocationZ();
	try {
	    MapObject target1 = m.getCell(destX, destY, destZ, MapConstants.LAYER_GROUND);
	    MapObject target2 = m.getCell(destX, destY, destZ, MapConstants.LAYER_OBJECT);
	...
	if (this.stateChanged) {
	    ...
	    app.getScenarioManager().getMap().addLevel(Support.getGameMapSize(), Support.getGameMapSize(),
		    Support.getGameMapFloorSize());
	    app.getScenarioManager().getMap().fillLevelRandomly(new Tile(), new Empty());
	    ...
	    if (didMapExist) {
		app.getScenarioManager().getMap().setGeneratorRandomness(currRandom, Gemma.GENERATOR_RANDOMNESS_MAX);
	    }
	    this.resetViewingWindowAndPlayerLocation();
	    Map m = app.getScenarioManager().getMap();
	    int px = m.getPlayerLocationX();
	...
	this.gameGUI.showOutput(Gemma.getApplication().getScenarioManager().getMap().getPlayerLocationZ());
    }
