...
class SaveTask extends Thread {
	...
	try {
	    // Set prefix handler
	    app.getScenarioManager().getMap().setXPrefixHandler(new PrefixHandler());
	    // Set suffix handler
	    app.getScenarioManager().getMap().setXSuffixHandler(new SuffixHandler());
	    app.getScenarioManager().getMap().writeMapX();
	    ZipUtilities.zipDirectory(new File(Support.getScenario().getBasePath()), mapFile);
