...
public class ScenarioManager {
	...
	if (frame != null) {
	    frame.getRootPane().putClientProperty("Window.documentModified", Boolean.valueOf(this.isDirty));
	}
	...
	if (saved) {
		...
		String save = CommonDialogs.showInputDialog("Open Which Saved Game?", title, saveList, saveList[0]);
		if (save != null) {
		    final File file = new File(SaveRegistration.getSaveBasePath() + File.separator + save
	...
	if (!FilenameChecker
		.isFilenameOK(ScenarioManager.getNameWithoutExtension(ScenarioManager.getFileNameOnly(filename)))) {
	    CommonDialogs.showErrorDialog("The file you selected contains illegal characters in its\n"
		    ...
		    + "named com1 through com9 and lpt1 through lpt9.", "Open Saved Game");
	} else {
	...
	if (app.getMode() == Application.STATUS_GAME) {
		...
		String extension = ScenarioManager.getExtension(this.lastUsedGameFile);
		if (extension != null) {
		    if (!(extension.equals(Extension.getGameExtension()))) {
			this.lastUsedGameFile = ScenarioManager.getNameWithoutExtension(this.lastUsedGameFile)
				+ Extension.getGameExtensionWithPeriod();
		    }
		} else {
	...
	while (!FilenameChecker.isFilenameOK(filename)) {
	    filename = CommonDialogs.showTextInputDialog("Saved Game Name:", title);
	    if (filename != null) {
		if (!FilenameChecker.isFilenameOK(filename)) {
		    CommonDialogs.showErrorDialog("The file name you entered contains illegal characters.\n"
			    ...
			    + "Files named con, nul, or prn are illegal, as are files\n"
			    + "named com1 through com9 and lpt1 through lpt9.", title);
		} else {
		    SaveRegistration.autoregisterSave(filename);
		    String dir = SaveRegistration.getSaveBasePath() + File.separator;
		    extension = ScenarioManager.getExtension(filename);
		    if (app.getMode() == Application.STATUS_GAME) {
			if (extension != null) {
			    if (!(extension.equals(Extension.getGameExtension()))) {
				filename = ScenarioManager.getNameWithoutExtension(filename)
					+ Extension.getGameExtensionWithPeriod();
			    }
			} else {
