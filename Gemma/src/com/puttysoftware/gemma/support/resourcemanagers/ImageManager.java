...
public class ImageManager {
    ...
    private static BufferedImageIcon getImage(final String name, final String rawName, final String cat,
	    final TemplateTransform tt, final boolean horzflip, final boolean vertflip, final boolean helpImage) {
	return ImageCache.getCachedImage(name, rawName, cat, tt, horzflip, vertflip, helpImage);
    }

    public static BufferedImageIcon getMapImage(final String name, final String rawName, final TemplateTransform tt) {
	return ImageManager.getImage(name, rawName, "objects", tt, false, false, false);
    }

    public static BufferedImageIcon getMapHelpImage(final String name, final String rawName,
	    final TemplateTransform tt) {
	return ImageManager.getImage(name, rawName, "objects", tt, false, false, true);
    }

    public static BufferedImageIcon getBossImage() {
	return ImageManager.getImage("boss", "boss", "boss", null, false, false, false);
    }

    public static BufferedImageIcon getMonsterImage(final String name, final Element element) {
	return ImageManager.getImage(name, name, "monsters", new TemplateTransform(element), false, false, false);
    }
	...
	return ImageManager.getImage(name, name, "players", new TemplateTransform(element), false, false, false);
    }

    public static BufferedImageIcon getStatImage(final String name) {
	return ImageManager.getImage(name, name, "stats", null, false, false, true);
    }

    static BufferedImageIcon getUncachedImage(final String name, final String cat, final TemplateTransform tt,
	...
	try {
	    URL url = ImageManager.LOAD_CLASS
		    .getResource(ImageManager.INTERNAL_LOAD_PATH + cat + "/" + normalName + ".png");
