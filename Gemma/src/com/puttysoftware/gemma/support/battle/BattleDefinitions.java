...
public class BattleDefinitions {
    ...
    private Creature pickFriendOfTeamInternal(String[] pickNames, int current, int number) {
	String text;
	if (number > 1) {
	...
	String response = CommonDialogs.showInputDialog(text + " - " + current + " of " + number, "Battle", pickNames,
		pickNames[0]);
	if (response != null) {
    ...
    private Creature pickEnemyOfTeamInternal(String[] pickNames, int current, int number) {
	String text;
	if (number > 1) {
