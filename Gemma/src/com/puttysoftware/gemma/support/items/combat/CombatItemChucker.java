...
public class CombatItemChucker {
    ...
    private static final Creature SOURCE = PartyManager.getNewPCInstance(0, 0, 0, 0, "");
    ...
    public static boolean selectAndUseItem(final Creature user, int teamID, boolean aiEnabled,
	    BattleDefinitions battle) {
	boolean result = false;
	CombatItem i = CombatItemChucker.selectItem(user);
	if (i != null) {
	    result = CombatItemChucker.useItem(i, user, teamID, aiEnabled, battle);
	}
	return result;
    }
	...
	if (ii != null) {
		...
		if (dialogResult != null) {
		    ...
		    CombatItem i = CombatItemChucker.COMBAT_ITEMS.getItemByName(names[index]);
		    if (ii.getUses(i) > 0) {
			return i;
		    } else {
    ...
    public static boolean useItem(final CombatItem used, final Creature user, int teamID, boolean aiEnabled,
	    BattleDefinitions battle) {
	if (used != null) {
	    ...
	    used.use();
	    e.resetEffect();
	    Creature[] targets = CombatItemChucker.resolveTarget(used, user, teamID, aiEnabled, battle);
    ...
    private static Creature[] resolveTarget(final CombatItem used, final Creature user, int teamID, boolean aiEnabled,
	    BattleDefinitions battle) {
