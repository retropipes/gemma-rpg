...
public class Shop implements ShopTypes {
	...
	int cost = 15 * i * i + 15 * i + (int) (Math.sqrt(Shop.getEquipmentCost(x)));
	if (cost < 0) {
	    return 0;
	} else {
	    return cost;
	...
	if (this.typeChoices != null) {
	    this.typeResult = CommonDialogs.showInputDialog("Select Type", this.getShopNameFromType(), this.typeChoices,
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
		...
		int handDefault = 0;
		String handResult = CommonDialogs.showInputDialog("Select Hand", this.getShopNameFromType(),
	...
	this.result = CommonDialogs.showInputDialog("Select", this.getShopNameFromType(), this.choices,
	...
	if ((this.type == ShopTypes.SHOP_TYPE_WEAPONS) || (this.type == ShopTypes.SHOP_TYPE_ARMOR)) {
	    ...
	    Equipment old = playerCharacter.getItems().getEquipmentInSlot(this.index);
	    if (old != null) {
		if (old.isTwoHanded()) {
		    this.twoHanded = true;
		}
		...
		this.item = EquipmentFactory.createFaithPoweredEquipment(old, this.typeIndex, bonus);
		this.cost = Shop.getEnhancementCost(bonus, power);
		if (this.cost == 0) {
	...
	// Confirm
	final int stage4Confirm = CommonDialogs
		.showConfirmDialog("This will cost " + this.cost + " Gold. Are you sure?", this.getShopNameFromType());
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
	    ...
	    if (this.typeResult.equals(this.typeChoices[0])) {
		Equipment bought = EquipmentFactory.createOneHandedWeapon(this.index, 0);
		playerCharacter.getItems().equipOneHandedWeapon(playerCharacter, bought, this.handIndex, true);
	    } else {
		Equipment bought = EquipmentFactory.createTwoHandedWeapon(this.index, 0);
		playerCharacter.getItems().equipTwoHandedWeapon(playerCharacter, bought, true);
	    }
	} else if (this.type == ShopTypes.SHOP_TYPE_ARMOR) {
	    playerCharacter.offsetGold(-this.cost);
	    Equipment bought = EquipmentFactory.createArmor(this.index, this.typeIndex, 0);
	    ...
	    playerCharacter.offsetGold(-this.cost);
	    playerCharacter.getItems().setEquipmentInSlot(this.index, (Equipment) this.item);
	    if (this.twoHanded) {
