...
class LayeredTower implements Cloneable {
	...
	LayeredTower copy = new LayeredTower(this.getRows(), this.getColumns(), this.getFloors());
	copy.data = new LowLevelDataStore(this.data);
	...
	if (this.noteData != null) {
	    copy.noteData = new LowLevelNoteDataStore(this.noteData);
	}
	...
	globalScriptArea.setLowerRight(new Point(this.getRows() - 1, this.getColumns() - 1));
	InternalScriptEntry act0 = new InternalScriptEntry();
	act0.setActionCode(InternalScriptActionCode.RANDOM_CHANCE);
	act0.addActionArg(new InternalScriptEntryArgument(Math.max(1000 + (mod * 200), 0)));
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    MapObject mo = this.getCell(y, x, z, MapConstants.LAYER_OBJECT);
		    if (mo != null) {
	...
	do {
	    ...
	    try {
		MapObject obj = this.getCell(fx1, fy1, zLoc, MapConstants.LAYER_OBJECT);
    ...
    private void fillFloorRandomly(final Map map, final int z, final int w, final MapObject pass1FillBottom,
	    final MapObject pass1FillTop) {
	// Pre-Pass
	...
	for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
	    MapObject[] objectsWithoutPrerequisites = objects.getAllNotRequired(e);
		...
		for (x = 0; x < columns; x++) {
			...
			if (e == MapConstants.LAYER_GROUND) {
				...
				for (y = 0; y < rows; y += this.regionSize) {
				    MapObject placeObj = objectsWithoutPrerequisites[r.generate()];
				    boolean okay = placeObj.shouldGenerateObject(map, y, x, z, w, e);
				    if (okay) {
					for (u = 0; u < this.regionSize; u++) {
					    for (v = 0; v < this.regionSize; v++) {
						this.setCell(objects.getNewInstanceByName(placeObj.getName()), v + x,
							u + y, z, e);
					    }
					}
				    }
				...
				for (y = 0; y < rows; y++) {
				    ...
				    boolean okay = placeObj.shouldGenerateObject(map, y, x, z, w, e);
				    if (okay) {
					this.setCell(objects.getNewInstanceByName(placeObj.getName()), x, y, z, e);
				    }
				}
			    }
			}
    ...
    public void fillRandomlyInBattle(final Map map, final int w, final MapObject pass1FillBottom,
	    final MapObject pass1FillTop) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorRandomlyInBattle(map, z, w, pass1FillBottom, pass1FillTop);
	}
    }

    private void fillFloorRandomlyInBattle(final Map map, final int z, final int w, final MapObject pass1FillBottom,
	    final MapObject pass1FillTop) {
	final int battleRegionSize = this.regionSize;
	// Pre-Pass
	final MapObjectList objects = new MapObjectList();
	RandomRange r = null;
	int x, y, e, u, v;
	...
	int rows = this.getRows();
	for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
	    MapObject[] objectsWithoutPrerequisites = objects.getAllNotRequiredInBattle(e);
	    if (objectsWithoutPrerequisites != null) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			if (e == MapConstants.LAYER_GROUND) {
			    for (x = 0; x < columns; x += battleRegionSize) {
				for (y = 0; y < rows; y += battleRegionSize) {
				    MapObject placeObj = objectsWithoutPrerequisites[r.generate()];
				    boolean okay = placeObj.shouldGenerateObjectInBattle(map, y, x, z, w, e);
				    if (okay) {
					for (u = 0; u < battleRegionSize; u++) {
					    for (v = 0; v < battleRegionSize; v++) {
						this.setCell(objects.getNewInstanceByName(placeObj.getName()), v + x,
							u + y, z, e);
					    }
					}
	...
	for (int layer = 0; layer < MapConstants.LAYER_COUNT; layer++) {
		...
		for (x = 0; x < requiredObjects.length; x++) {
			...
			if (currObj.shouldGenerateObject(map, randomRow, randomColumn, z, w, layer)) {
			    ...
			    while (!currObj.shouldGenerateObjectInBattle(map, randomColumn, randomRow, z, w, layer)) {
				randomRow = row.generate();
				randomColumn = column.generate();
			    }
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
	...
	for (InternalScriptArea isa : this.scriptAreas) {
	    if (p.x >= isa.getUpperLeft().x && p.x <= isa.getLowerRight().x && p.y >= isa.getUpperLeft().y
		    && p.y <= isa.getLowerRight().y && this.getPlayerFloor() == z) {
		retVal.add(isa);
	    }
	}
	return retVal;
    ...
    public static LayeredTower readXLayeredTower(XDataReader reader, int ver) throws IOException {
    ...
    public void readSavedTowerStateX(XDataReader reader, int formatVersion) throws IOException {
