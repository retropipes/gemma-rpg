...
class SeekerAI extends AIRoutine {
	...
	if (this.spellCheck(ac)) {
		...
		for (int x = 2; x <= 9; x++) {
			...
			if (this.lastResult == false) {
			    ...
			    RandomRange randTurn = new RandomRange(0, 1);
			    int rt = randTurn.generate();
			    if (rt == 0) {
				there = SeekerAI.turnRight45(this.moveX, this.moveY);
			    } else {
				there = SeekerAI.turnLeft45(this.moveX, this.moveY);
			    }
			    this.moveX = there.x;
			    this.moveY = there.y;
			} else {
    ...
    private static Point turnRight45(int x, int y) {
	if (x == -1 && y == -1) {
    ...
    private static Point turnLeft45(int x, int y) {
	if (x == -1 && y == -1) {
	    return new Point(-1, 0);
	} else if (x == -1 && y == 0) {
	    return new Point(-1, 1);
    ...
    private boolean spellCheck(AIContext ac) {
	RandomRange random = new RandomRange(1, 100);
	int chance = random.generate();
	if (chance <= SeekerAI.CAST_SPELL_CHANCE) {
	    int maxIndex = SeekerAI.getMaxCastIndex(ac);
	    if (maxIndex > -1) {
		if (ac.getCharacter().getCurrentSP() > 0) {
		    // Select a random spell to cast
		    RandomRange randomSpell = new RandomRange(0, maxIndex);
		    int randomSpellID = randomSpell.generate();
    ...
    private static int getMaxCastIndex(AIContext ac) {
	int currMP = ac.getCharacter().getTemplate().getCurrentMP();
	int[] allCosts = ac.getCharacter().getTemplate().getSpellBook().getAllSpellCosts();
	int result = -1;
	if (currMP > 0) {
    ...
    private static boolean stealCheck(AIContext ac) {
	RandomRange random = new RandomRange(1, 100);
	int chance = random.generate();
    ...
    private static boolean drainCheck(AIContext ac) {
	RandomRange random = new RandomRange(1, 100);
	int chance = random.generate();
