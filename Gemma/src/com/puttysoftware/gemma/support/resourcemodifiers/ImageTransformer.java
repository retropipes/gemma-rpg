 ...
 */
package com.puttysoftware.gemma.support.resourcemodifiers;
...
public class ImageTransformer {
    ...
    public static BufferedImageIcon getTemplateTransformedImage(final BufferedImageIcon icon,
    ...
    public static BufferedImageIcon getTransformedImage(final BufferedImageIcon icon) {
	try {
	    BufferedImageIcon result = new BufferedImageIcon(icon);
	    if (icon != null) {
		for (int x = 0; x < icon.getWidth(); x++) {
    ...
    public static BufferedImageIcon getHorizontallyFlippedImage(final BufferedImageIcon icon) {
	try {
	    BufferedImageIcon result = new BufferedImageIcon(icon);
	    for (int x = 0; x < icon.getWidth(); x++) {
		for (int y = 0; y < icon.getHeight(); y++) {
    ...
    public static BufferedImageIcon getVerticallyFlippedImage(final BufferedImageIcon icon) {
	try {
	    BufferedImageIcon result = new BufferedImageIcon(icon);
	    for (int x = 0; x < icon.getWidth(); x++) {
		for (int y = 0; y < icon.getHeight(); y++) {
    ...
    public static BufferedImageIcon getCompositeImage(final BufferedImageIcon... icons) {
	try {
	    if (icons.length == 2) {
		...
		if (icon1 != null && icon2 != null) {
		    if (icon1.getWidth() == icon2.getWidth() && icon1.getHeight() == icon2.getHeight()) {
		...
		BufferedImageIcon result = ImageTransformer.getCompositeImage(icons[0], icons[1]);
		if (result != null) {
		    for (int x = 2; x < icons.length; x++) {
			result = ImageTransformer.getCompositeImage(result, icons[x]);
			if (result == null) {
			    return null;
			}
		    }
