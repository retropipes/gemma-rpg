...
public class PartyMember extends Creature {
	...
	act0.addActionArg(new InternalScriptEntryArgument(PartyManager.getParty().getDungeonLevel() - this.getLevel()));
    ...
    private void loadPartyMember(final int newLevel, final int chp, final int cmp, final int newGold, final int newLoad,
	    final long newExperience, final boolean[] known, final long[] pres) {
	...
	return (int) (super.getActionsPerRound()
		* this.getPersonality().getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_MOVE_MOD));
    }
