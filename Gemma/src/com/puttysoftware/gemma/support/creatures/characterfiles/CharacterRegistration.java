...
public class CharacterRegistration {
	...
	// Load character list
	String[] characterNameList = CharacterRegistration.getCharacterNameList();
	String[] characterNames = new File(CharacterRegistration.getBasePath()).list(new CharacterFilter());
	if (characterNames != null && characterNames.length > 0) {
	    // Strip extension
	    int stripCount = Extension.getCharacterExtensionWithPeriod().length();
	    ...
	    String res = CommonDialogs.showInputDialog("Register Which Character?", "Register Character" + suffix,
		...
		if (!alreadyRegistered) {
			...
			if (CharacterRegistration.ANY_FOUND && characterNameList != null) {
			    String[] newCharacterList = new String[characterNameList.length + 1];
	...
	// Check for null list
	if (characterNameList == null) {
	    CommonDialogs.showTitledDialog("No Characters Registered!", "Unregister Character" + suffix);
	    return;
	}
	// Pick character to unregister
	String res = CommonDialogs.showInputDialog("Unregister Which Character?", "Unregister Character" + suffix,
	...
	// Check for null list
	if (characterNameList == null) {
	    CommonDialogs.showTitledDialog("No Characters Registered!", "Remove Character" + suffix);
	    return;
	}
	// Pick character to unregister
	String res = CommonDialogs.showInputDialog("Remove Which Character?", "Remove Character" + suffix,
		characterNameList, characterNameList[0]);
	if (res != null) {
	    // Find character index
	    int index = -1;
	...
	ArrayList<String> registeredNames = CharacterRegistration.readCharacterRegistry();
