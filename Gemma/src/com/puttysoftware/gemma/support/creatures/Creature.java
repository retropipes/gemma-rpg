...
public abstract class Creature {
	...
	long damageGiven = this.getPrestigeValue(PrestigeConstants.PRESTIGE_DAMAGE_GIVEN);
	long damageTaken = this.getPrestigeValue(PrestigeConstants.PRESTIGE_DAMAGE_TAKEN);
	long hitsGiven = this.getPrestigeValue(PrestigeConstants.PRESTIGE_HITS_GIVEN);
	long hitsTaken = this.getPrestigeValue(PrestigeConstants.PRESTIGE_HITS_TAKEN);
	long attacksDodged = this.getPrestigeValue(PrestigeConstants.PRESTIGE_ATTACKS_DODGED);
	long missedAttacks = this.getPrestigeValue(PrestigeConstants.PRESTIGE_MISSED_ATTACKS);
	long monstersKilled = this.getPrestigeValue(PrestigeConstants.PRESTIGE_MONSTERS_KILLED);
	long spellsCast = this.getPrestigeValue(PrestigeConstants.PRESTIGE_SPELLS_CAST);
	long timesKilled = this.getPrestigeValue(PrestigeConstants.PRESTIGE_TIMES_KILLED);
	long timesRanAway = this.getPrestigeValue(PrestigeConstants.PRESTIGE_TIMES_RAN_AWAY);
	return (damageGiven - damageTaken) / 10 + hitsGiven + attacksDodged + (2 * monstersKilled) - hitsTaken
		- (2 * missedAttacks) - (3 * spellsCast) - (10 * timesKilled) - (50 * timesRanAway);
    }
	...
	for (x = 0; x < this.effectList.length; x++) {
	    ...
	    } catch (final NullPointerException | ArrayIndexOutOfBoundsException np) {
		// Do nothing
	    }
	}
	return c;
	...
	int value = (int) Math.sqrt(Math.ceil(
	...
	return (int) (this.getIntelligence() * StatConstants.FACTOR_INTELLIGENCE_MAGIC_POWER);
    }

    public final String getMagicPowerString() {
	return " Magic Power: " + this.getEffectedStat(StatConstants.STAT_MAGIC_POWER) + " (" + this.getMagicPower()
		+ ")";
    }
	...
	return (int) (this.getBlock() * StatConstants.FACTOR_BLOCK_MAGIC_DEFENSE);
    }

    public final String getMagicDefenseString() {
	return "Magic Defense: " + this.getEffectedStat(StatConstants.STAT_MAGIC_DEFENSE) + " ("
	...
	double agilityContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_AGILITY))
		* StatConstants.FACTOR_AGILITY_EVADE;
	double luckContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_LUCK))
	...
	double strengthContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_STRENGTH))
		* StatConstants.FACTOR_STRENGTH_HIT;
	double luckContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_LUCK)) * StatConstants.FACTOR_LUCK_HIT;
    ...
    public static long getAdjustedExperience(long baseExp, int baseLevel, int otherLevel) {
	return Math.max((long) (baseExp + ((otherLevel - baseLevel) * baseExp * Creature.EXP_ADJUST)), 0);
    }
