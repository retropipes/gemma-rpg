...
public class Party {
	...
	for (int x = 0; x < this.battlers.length; x++) {
		...
		CommonDialogs.showTitledDialog(this.battlers[x].getTemplate().getName() + " reached level "
			+ this.battlers[x].getTemplate().getLevel() + "!", "Level Up");
		...
		act0.addActionArg(new InternalScriptEntryArgument(
			Math.max(1, (10 * this.battlers[x].getTemplate().getLevel() - 1) / this.activePCs)));
    ...
    public void distributeVictorySpoils(VictorySpoilsDescription vsd, int otherLevel) {
	...
	for (int x = 0; x < divMod; x++) {
		...
		this.battlers[x].getTemplate().offsetExperience(
			Creature.getAdjustedExperience(vsd.getExpPerMonster(y), this.getLeader().getLevel(), otherLevel)
				/ divMod);
	    }
	    this.battlers[x].getTemplate().offsetGold(vsd.getGoldWon() / divMod);
	}
    }
	...
	String response = CommonDialogs.showInputDialog("Pick 1 Party Member", "Create Party", pickNames, pickNames[0]);
	if (response != null) {
