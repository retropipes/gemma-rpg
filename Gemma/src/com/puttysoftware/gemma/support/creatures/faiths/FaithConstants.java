...
public class FaithConstants {
	...
	return NamesManager.getName(NamesConstants.SECTION_FAITH_POWERS_PREFIX + FaithConstants.FAITH_NAMES[f],
	...
	if (!FaithConstants.INITED) {
	    try (final ResourceStreamReader rsr1 = new ResourceStreamReader(FaithDataManager.class
		    .getResourceAsStream("/com/puttysoftware/gemma/support/resources/data/faith/catalog.txt"))) {
		// Fetch data
	    ...
	    try (final ResourceStreamReader rsr2 = new ResourceStreamReader(FaithDataManager.class
		    .getResourceAsStream("/com/puttysoftware/gemma/support/resources/data/faith/colors.txt"))) {
		...
		String[] tempColors2 = tempColors.toArray(new String[tempColors.size()]);
