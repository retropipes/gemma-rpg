...
public class PartyManager {
	...
	int mem = 0;
	PartyMember[] pickMembers = CharacterLoader.loadAllRegisteredCharacters();
	...
	for (int x = 0; x < PartyManager.PARTY_SIZE; x++) {
		...
		int response = CommonDialogs.showConfirmDialog("Add another character?", "Create Party");
    ...
    public static PartyMember getNewPCInstance(int r, int f, int p, int g, String n) {
	...
	String name = CommonDialogs.showTextInputDialog("Character Name", "Create Character");
	if (name != null) {
	    Race race = RaceManager.selectRace(owner);
	    if (race != null) {
		Faith faith = FaithManager.selectFaith(owner);
		if (faith != null) {
		    Personality personality = PersonalityManager.selectPersonality(owner);
		    if (personality != null) {
			Gender gender = GenderManager.selectGender();
			if (gender != null) {
			    return new PartyMember(race, faith, personality, gender, name);
			}
		    }
		}
	    }
    ...
    public static String showCreationDialog(JFrame owner, String labelText, String title, String[] input,
	    String[] descriptions) {
